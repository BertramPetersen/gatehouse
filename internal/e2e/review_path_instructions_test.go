...
// trustedRepoConfigWithPathInstructions is the .gatehouse.yaml a maintainer
...
// pushedRepoConfigAttemptingToSteerReview is the .gatehouse.yaml a contributor
...
func TestReviewPathInstructionsJourney(t *testing.T) {
	...
	t.Run("matching_trusted_rule_reaches_the_reviewer", func(t *testing.T) {
		...
		// glob, and the contributor's own .gatehouse.yaml.
		branch := "scoped-review-matched"
		h.CommitChange(branch, "internal/scm/github/github.go", "package github\n\n// changed\n", "touch scm")
		h.CommitChange(branch, "notes.txt", "unrelated note\n", "touch notes")
		h.CommitChange(branch, ".gatehouse.yaml", pushedRepoConfigAttemptingToSteerReview,
...
// pushMainRepoConfig commits a new trusted default-branch .gatehouse.yaml and
...
func pushMainRepoConfig(t *testing.T, h *Harness, yaml string) {
	t.Helper()
	h.CommitChange("main", ".gatehouse.yaml", yaml, "maintainer: configure review path instructions")
