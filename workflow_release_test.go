...
func TestReleaseWorkflowKeepsTelemetryOptIn(t *testing.T) {
	data, err := os.ReadFile(".github/workflows/release.yml")
	if err != nil {
		t.Fatalf("read workflow: %v", err)
	}
	...
	block := extractJobBlock(t, string(data), "build-and-upload")
	// Telemetry is opt-in: the repo variables render empty until an operator
	// sets them, and an empty website ID makes telemetry.Default() a noopSink.
