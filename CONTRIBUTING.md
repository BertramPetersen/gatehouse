...
**All pull requests to this repository must be raised through `gatehouse`.**

This repo _is_ gatehouse. Contributions should be done using the tool itself, which reduces the maintainer's burden of reviewing and merging contributions.
The `Require gatehouse` GitHub Actions workflow runs on every PR and fails if the body is missing the deterministic signature and structured pipeline step attestation that gatehouse writes. PRs without them will not be reviewed or merged.
If you revise the PR description after gatehouse creates it, preserve the generated `## Pipeline` section. Replacing the whole body removes the signature or attestation and makes the required check fail until gatehouse writes the section again.
...
1. Fork the repo, then clone the parent repo or set your local `origin` back to the parent repo (`git@github.com:BertramPetersen/gatehouse.git`).
2. Create a branch and make your changes.
3. Initialize or refresh the gate with your fork as the push target: `gatehouse init --fork-url git@github.com:<you>/gatehouse.git`.
...
6. Run `gatehouse` to attach to the pipeline, watch findings, and auto-fix or review as needed.
...
- Run `make skill` when you change the canonical agent skill content under `internal/skill`; `make lint` fails if any committed gatehouse skill file has drifted.
