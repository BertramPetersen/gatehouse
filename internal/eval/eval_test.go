...
func TestCaptureStripsEveryHarnessPinFromThePinnedConfig(t *testing.T) {
	...
	pinned = append(pinned, []byte("agent_profiles:\n  thorough:\n    codex: {model: large}\nagent_step_profiles: {review: thorough}\n")...)
	neutral, err := agentNeutralGlobalConfig(pinned)
	if err != nil {
		t.Fatal(err)
	}
	for _, key := range []string{"agent:", "agent_args_override", "agent_config", "agent_profiles", "agent_step_profiles"} {
