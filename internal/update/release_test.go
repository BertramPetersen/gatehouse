...
func TestPickReleaseAssets(t *testing.T) {
	assets := []releaseAsset{
		{Name: "gatehouse-v1.2.3-darwin-arm64.tar.gz", BrowserDownloadURL: "https://example.com/archive"},
		{Name: "checksums.txt", BrowserDownloadURL: "https://example.com/checksums"},
		{Name: "ignored.txt", BrowserDownloadURL: "https://example.com/ignored"},
	}

	archive, checksums, err := pickReleaseAssets("gatehouse", "v1.2.3", assets, platformSpec{GOOS: "darwin", GOARCH: "arm64"})
	if err != nil {
		t.Fatalf("pickReleaseAssets error = %v", err)
	}
	if archive.Name != "gatehouse-v1.2.3-darwin-arm64.tar.gz" {
	...
	_, _, err = pickReleaseAssets("gatehouse", "v1.2.3", assets[:1], platformSpec{GOOS: "darwin", GOARCH: "arm64"})
	if err == nil {
...
func TestParseChecksums(t *testing.T) {
	...
	data := []byte("abc123  gatehouse-v1.2.3-darwin-arm64.tar.gz\ndef456  other.tar.gz\n")
