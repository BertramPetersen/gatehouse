...
func TestPushWithOptionsForwardsPushOptions(t *testing.T) {
	...
	if err := PushWithOptions(ctx, src, "dest", "refs/heads/main", "", false, []string{"gatehouse.skip=test,lint"}); err != nil {
	...
	if got, want := strings.TrimSpace(string(data)), "1:gatehouse.skip=test,lint"; got != want {
