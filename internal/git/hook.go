...
const gateConfigStampFile = "gatehouse-gate-config"
const preservedPreReceiveHook = "pre-receive.gatehouse-user"
...
func preReceiveHookScript(command string) string {
	return `#!/bin/sh
# gatehouse pre-receive hook
# Authorize the pushing process before any managed gate ref changes.
GATEHOUSE_BIN=` + shellSingleQuote(command) + `
if [ ! -f "$GATEHOUSE_BIN" ]; then
  GATEHOUSE_BIN="$(command -v gatehouse 2>/dev/null || echo gatehouse)"
fi
...
out=$(GATEHOUSE_HOOK_HELPER=1 "$GATEHOUSE_BIN" daemon admit-push --gate "$GATE_DIR" 2>&1)
status=$?
if [ $status -ne 0 ]; then
  printf 'gatehouse: gate push refused before ref mutation:\n%s\n' "$out" >&2
  exit $status
fi
...
func isManagedPreReceiveHook(content []byte) bool {
	text := string(content)
	return strings.Contains(text, "# gatehouse pre-receive hook") && strings.Contains(text, "daemon admit-push")
}
...
while read oldrev newrev refname; do
	  ...
	  out=$(GATEHOUSE_HOOK_HELPER=1 "$GATEHOUSE_BIN" daemon notify-push "$@" 2>&1)
  status=$?
  if [ $status -ne 0 ]; then
    notify_failed=1
    {
    ...
    {
      printf 'gatehouse: notify-push failed for %s (exit %d):\n' "$refname" "$status"
      printf '%s\n' "$out"
...
if [ "$notify_failed" -eq 0 ]; then
  cat >&2 <<'BANNER'
____ ____ ___ ____ _  _ ____ _  _ ____ ____
| __ |__|  |  |___ |__| |  | |  | [__  |___
...
func isManagedPostReceiveHook(content []byte) bool {
	text := string(content)
	return strings.Contains(text, "# gatehouse post-receive hook") && strings.Contains(text, "daemon notify-push")
}
...
// RefreshManagedPostReceiveHook updates an existing gatehouse-owned hook.
