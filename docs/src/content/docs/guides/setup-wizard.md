---
title: Setup Wizard
description: What bare `gatehouse` does when there's no active run on the current branch.
---

When you run `gatehouse` with no arguments and there is no active run on the
current branch, `gatehouse` can walk you through creating a branch,
...
The point of the wizard is to make bare `gatehouse` a useful starting
...
- You're in an interactive terminal, or you passed `gatehouse -y` / `gatehouse --yes`.
- The gate is initialized for the current repo (`gatehouse init` has been run).
- There's no active run on the current branch.

In non-interactive contexts, bare `gatehouse` without `-y` falls back to listing the last 5 runs instead.
...
Pass `--skip` to skip comma-separated pipeline steps for the new run created by the wizard, for example `gatehouse --skip test,lint`.
It only applies when the wizard starts a new pipeline run; if bare `gatehouse` attaches to an active run or lists recent runs, `--skip` exits with an error.

If you want to attach to *any* active run in the repo (not just the current branch), use `gatehouse attach` - that path skips the wizard entirely.

## Wizard flow

```mermaid
flowchart TD
  start["Run gatehouse"] --> active{"Active run on current branch?"}
...
Always shown. Asks whether to push the current branch to the `gatehouse` gate.
...
The managed agent server (Rovo Dev or OpenCode) writes its output to `~/.gatehouse/logs/wizard-agent.log` during these runs.

## Environment sanity

The wizard requires:

- The gate to be initialized (`gatehouse init` has run).
- A clean enough state to commit and push.
- A configured native or ACP agent, only when the wizard needs to suggest a branch name or commit subject. For an ordered fallback list, at least one configured entry must be available. See the [Global Config Reference](/gatehouse/reference/global-config/) for ACP aliases such as `agent: cursor` and target requirements.
...
`gatehouse doctor` is the fastest way to check whether the configured global runner can start a validation gate.
