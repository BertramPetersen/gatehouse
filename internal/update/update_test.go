...
func TestUpdaterCheckLatestAndRefreshCache(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			archiveName: "gatehouse-v1.2.3-darwin-arm64.tar.gz",
		},
		{
			name:        "windows zip",
			platform:    platformSpec{GOOS: "windows", GOARCH: "amd64"},
			archiveName: "gatehouse-v1.2.3-windows-amd64.zip",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.URL.Path != "/repos/BertramPetersen/gatehouse/releases/latest" {
...
func TestUpdaterRunReplacesExecutable(t *testing.T) {
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/repos/BertramPetersen/gatehouse/releases/latest":
	...
	if !strings.Contains(stdout.String(), "updated gatehouse") {
...
func TestUpdaterRunFailsWhenDaemonUsesDifferentExecutable(t *testing.T) {
	...
	execDir := t.TempDir()
	execPath := filepath.Join(execDir, "gatehouse")
	if err := os.WriteFile(execPath, []byte("old-binary"), 0o755); err != nil {
		t.Fatal(err)
	}
	otherExecPath := filepath.Join(execDir, "other-gatehouse")
...
func TestUpdaterRunReplacesDaemonWhenDifferentExecutableConfirmed(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(stdout.String(), "updated gatehouse from v1.2.2 to v1.2.3") {
...
func TestUpdaterMaybeNotifyAndCheck(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "A new version of gatehouse is available: v1.2.2 -> v1.2.3") {
		t.Fatalf("stderr = %q", stderr.String())
	}
	if !spawned {
...
func TestUpdaterCheckLatestBetaUsesReleasesList(t *testing.T) {
	...
	archiveName := "gatehouse-v1.3.0-beta.1-darwin-arm64.tar.gz"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/repos/BertramPetersen/gatehouse/releases":
			fmt.Fprintf(w, `[
		...
		case "/repos/BertramPetersen/gatehouse/tags":
...
func TestUpdaterCheckLatestBetaPicksHighestSemver(t *testing.T) {
	...
	archiveName := "gatehouse-v1.3.0-beta.2-darwin-arm64.tar.gz"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/repos/BertramPetersen/gatehouse/releases":
			fmt.Fprintf(w, `[
...
func TestUpdaterCheckLatestBetaFallsBackToTagsWhenListingStale(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/repos/BertramPetersen/gatehouse/releases/tags/v1.3.0-beta.1":
...
func TestUpdaterCheckLatestBetaChecksListedReleaseAfterMissingTags(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		default:
			if strings.HasPrefix(r.URL.Path, "/repos/BertramPetersen/gatehouse/releases/tags/") {
				tagFetches++
				http.NotFound(w, r)
				return
			}
