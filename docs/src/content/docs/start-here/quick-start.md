---
title: Quick Start
description: Initialize gatehouse and run your first gated push.
---

This walks you through your first gated push. For install options other than the macOS/Linux one-liner, see [Installation](/gatehouse/start-here/installation/).
...
The installer drops the binary in `~/.gatehouse/bin`, links it into `~/.local/bin` or `/usr/local/bin`, and restarts the background daemon. If the restart fails, the install command fails.
...
- `git`
- One supported agent runner (`claude`, `codex`, `grok`, `acli` for Rovo Dev, `opencode`, `pi`, `copilot`, or `agy` for Antigravity), or a configured Cursor/ACP runner such as `agent: cursor`; see [Global Config](/gatehouse/reference/global-config/) for ACP requirements
...
`gatehouse doctor` reports whether the configured global runner can start a validation gate.
Every validation gate requires a runnable pipeline agent and otherwise fails before its first pipeline step.

See [Provider Integration](/gatehouse/guides/provider-integration/) for PR/CI setup.
...
Instead of `git push origin`, push to the `gatehouse` remote:
...
If the current branch has an active run, this attaches directly. If not, the setup wizard can walk you through creating a branch, committing, and pushing through the gate, then attach if the daemon registers the new run. By default that path is interactive in a TTY. With `gatehouse -y`, the wizard accepts defaults automatically, stays visible and auto-advances in a TTY, and falls back to the headless path without a TTY.

The TUI shows each step's progress, streams agent output, and pauses for your approval when findings need attention. See [Using the TUI](/gatehouse/guides/tui/) for keybindings and layout.
...
`gatehouse init` installed the `/gatehouse` skill at user level, available in every repo, so you can ask the agent to do a task and gate it:

```
/gatehouse add a --json flag to the status command
```

Or, if the work is already committed on a feature branch, use bare `/gatehouse` to validate it:
...
It drives the same gate as the TUI through `gatehouse axi`, a non-interactive command surface that uses flags only, prints TOON on stdout, and exposes the same approval gates through `gatehouse axi respond`.

See [Driving gatehouse as an agent](/gatehouse/guides/agents/#driving-gatehouse-as-an-agent) for the full agent workflow.
...
Steps that find issues pause for your approval. See the [Pipeline concept page](/gatehouse/concepts/pipeline/) for the overview and [Pipeline Steps](/gatehouse/reference/pipeline-steps/) for each step's exact behavior.
