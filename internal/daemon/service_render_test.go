...
func TestServiceDefinitionMatchesRootRejectsPrefixOnlyMatch(t *testing.T) {
	...
	if serviceDefinitionMatchesRoot([]byte(renderLaunchAgent("/opt/gatehouse/bin/gatehouse", other, home)), p) {
		t.Fatal("expected launch agent root prefix collision to be rejected")
	}
	if serviceDefinitionMatchesRoot([]byte(renderSystemdUnit("/usr/local/bin/gatehouse", other, home)), p) {
	...
	if !serviceDefinitionMatchesRoot([]byte(renderLaunchAgent("/opt/gatehouse/bin/gatehouse", p, home)), p) {
		t.Fatal("expected exact launch agent root match")
	}
	if !serviceDefinitionMatchesRoot([]byte(renderSystemdUnit("/usr/local/bin/gatehouse", p, home)), p) {
...
func TestRenderLaunchAgentIncludesManagedPath(t *testing.T) {
	...
	plist := renderLaunchAgent("/opt/gatehouse/bin/gatehouse", p, home)
...
func TestManagedServicesRouteRawOutputToBootstrapSink(t *testing.T) {
	...
	plist := renderLaunchAgent("/opt/gatehouse/bin/gatehouse", p, "/Users/test")
	...
	unit := renderSystemdUnit("/usr/local/bin/gatehouse", p, "/home/test")
