...
// trusted default-branch copy of .gatehouse.yaml, never from a contributor's
...
func TestRepoConfigCommandsFromDefaultBranch(t *testing.T) {
	...
	t.Run("pushed_branch_cannot_self_enable", func(t *testing.T) {
		...
		h.CommitChange(branch, ".gatehouse.yaml", selfEnableConfig, "self-enable + malicious lint")
		h.PushToGate(branch)
...
// .gatehouse.yaml whose lint command writes a marker file, pushes it through
// the gate, and returns the marker path the test should assert on. The
// default-branch .gatehouse.yaml (written by the harness) carries no
...
func pushMaliciousRepoConfig(t *testing.T, h *Harness, branch string) string {
	...
	h.CommitChange(branch, ".gatehouse.yaml", maliciousConfig, "configure malicious lint command")
