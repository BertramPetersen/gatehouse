...
// daemonProcessInfo describes a running `gatehouse daemon run --root <root>`
...
// daemonListDaemonProcesses enumerates gatehouse managed-daemon processes. It
...
// path spelling (e.g. a symlinked GATEHOUSE_HOME).
...
// looks like `gatehouse daemon run --root <root>`, extracting the pid and the
...
// looksLikeDaemonRunCommand reports whether a command line is a gatehouse
...
// reconcileCollidingDaemons detects gatehouse daemons already running for the
// same logical root as p but started under a different path spelling (e.g. a
// symlinked or relative GATEHOUSE_HOME). The socket-keyed health check in Start cannot
