...
func TestClientTrackSendsUmamiEventPayload(t *testing.T) {
	...
	case got := <-reqCh:
		...
		if got.Payload.URL != "app://gatehouse/command" {
			t.Fatalf("url = %q, want %q", got.Payload.URL, "app://gatehouse/command")
		}
		if got.Payload.Title != "gatehouse CLI" {
			t.Fatalf("title = %q, want %q", got.Payload.Title, "gatehouse CLI")
		}
