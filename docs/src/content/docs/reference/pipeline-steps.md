...
This is the per-step reference. For the overview and rationale, see [Pipeline](/gatehouse/concepts/pipeline/). For the fix loop, see [Auto-Fix Loop](/gatehouse/concepts/auto-fix/).
...
Beyond these core steps, a repository can declare extra checks that run immediately after one of them; [`gates`](/gatehouse/reference/repo-config/#gates) owns their placement, failure handling, and limits.
...
Configured shell commands and one-shot agent subprocesses are scoped to their step: when the invocation exits, fails, or is cancelled, gatehouse terminates remaining child processes it spawned so background workers do not outlive the run.
When configured Test or Lint command output exceeds 64 KiB, the complete output remains in the authoritative step log while findings, IPC responses, and repair prompts receive a valid-UTF-8 head-and-tail projection capped at 64 KiB. The truncation marker reports the exact original and omitted byte counts and points to `gatehouse axi logs --step <step> --full` for the complete output.
Commits created by the shared Review, Test, Document, and Lint fix path, plus CI repair commits, use the configurable [`commit.fix_message`](/gatehouse/reference/global-config/#commitfix_message) template.
...
When a human resolves a findings gate with Approve, Skip, or Abort without selecting a fix, gatehouse records that the round's findings were declined. A gate with no findings records no decision. When the human selects only some findings to fix, the unselected complement is recorded as declined; findings merely left out by automatic filtering remain undecided.
...
The integration branch used below is the [PR base branch](/gatehouse/reference/repo-config/#prbase_branch): the repository's forge default branch, or the trusted [`pr.base_branch`](/gatehouse/reference/repo-config/#prbase_branch) when configured.
...
- With GitHub fork routing, the pushed-branch target is the fork branch fetched into `refs/remotes/gatehouse-push/<branch>`
...
- Bounds the conflict-repair agent with [`agent_timeout`](/gatehouse/reference/global-config/#agent_timeout): an expired budget cancels the agent and fails the step with a timeout diagnostic rather than leaving the run active indefinitely
...
- Includes user intent when the run has supplied intent or transcript matching found a relevant local agent session; the detailed provenance semantics are documented in [Intent extraction](/gatehouse/guides/agents/#intent-extraction)
...
At entry to every remaining step in the fixed pipeline order - Test, Document, Lint, Push, PR, and CI - gatehouse compares the live worktree `HEAD` with the pipeline-recorded head. An equal head or a pipeline-descendant commit continues. A backward reset, divergent sibling, or unverifiable relationship fails the run before that step performs work, including for steps that would not create a commit.
...
[`commands.test`](/gatehouse/reference/repo-config/#commandstest) owns the configuration contract for any explicit baseline command.
...
- Evidence is always collected under the run's evidence directory (`<GATEHOUSE_HOME>/evidence/<run-id>` by default, see [`test.evidence`](/gatehouse/reference/global-config/#testevidence)), outside the worktree, so artifacts never enter the branch being validated. On GitHub, [`test.evidence.store_in_repo: true`](/gatehouse/reference/global-config/#testevidence) makes the PR step publish that directory to the push-target repository's orphan evidence branch under `<test.evidence.dir>/<branch-slug>` and link the artifacts from the PR body. The config reference owns provider support and fail-closed behavior.
...
- Bounds the documentation (and combined housekeeping) agent with [`agent_timeout`](/gatehouse/reference/global-config/#agent_timeout): an expired budget cancels the agent and fails the step with a timeout diagnostic rather than leaving the run active indefinitely
...
- Bounds those agent turns, including a configured-lint repair turn, with [`agent_timeout`](/gatehouse/reference/global-config/#agent_timeout): an expired budget cancels the agent and fails the step with a timeout diagnostic rather than leaving the run active indefinitely
...
- Commits any uncommitted changes left by pipeline agents or the formatter with message `gatehouse: apply agent fixes`
...
**Skipped when:**
- The branch is the [PR base branch](/gatehouse/reference/repo-config/#prbase_branch) (the repository's forge default branch, or the trusted `pr.base_branch` when configured)
...
- Bitbucket Cloud credentials are missing (`GATEHOUSE_BITBUCKET_EMAIL` or `GATEHOUSE_BITBUCKET_API_TOKEN`)
...
**Behavior:**
- Checks for an existing PR on the branch, matching by branch alone rather than filtering by base, so a still-open PR against a since-changed [`pr.base_branch`](/gatehouse/reference/repo-config/#prbase_branch) is found and updated instead of orphaned behind a duplicate
...
- Bounds the PR-drafting agent with [`agent_timeout`](/gatehouse/reference/global-config/#agent_timeout): an expired budget cancels the agent and uses that same fallback rather than leaving the run active indefinitely; a late successful title after the deadline is not used
...
Immediately after the existing `Updates from [git push gatehouse](https://github.com/BertramPetersen/gatehouse)` signature, gatehouse writes one stable HTML comment:

```html
<!-- gatehouse-pipeline-attestation:v1 {"head_sha":"0123456789abcdef0123456789abcdef01234567","steps":[{"step":"review","status":"completed"}]} -->
```

The `v1` payload is compact JSON with these required fields:

- `head_sha`: the exact git commit SHA recorded for the run when gatehouse writes the PR body
- `steps`: the ordered pipeline step snapshot; every item has exactly the fields below

- `step`: the raw pipeline step name, such as `intent`, `rebase`, `review`, `test`, `document`, `lint`, `push`, `pr`, or `ci`; a repository-declared [gate](/gatehouse/reference/repo-config/#gates) appears under its own step name, `gate.<anchor>.<name>`
- `status`: the raw [step status](#step-statuses) recorded for that step, such as `completed`, `skipped`, or `failed`

Items are ordered by the fixed pipeline order, with each repository gate immediately after the core step it is anchored to, and represent the exact database snapshot when gatehouse creates or updates the PR body. The attestation includes `pr` and `ci` records even though their human-readable details are not shown in `## Pipeline`; at the normal PR write point those records are commonly `running` and `pending`. The `head_sha` binds that snapshot to the commit it describes, so consumers can detect when a later push has made the comment stale. It is not refreshed after the PR step unless gatehouse writes the body again.

The comment is intentionally data only. It does not declare any step required, passed for a policy, compliant, or mergeable. Consumers can parse the versioned JSON without scraping prose and apply their own policy. The comment stays with the Pipeline header when gatehouse truncates older human-readable update details to fit a PR-body limit, and is omitted on Bitbucket Cloud.
...
- Bitbucket Cloud requires `GATEHOUSE_BITBUCKET_EMAIL` and `GATEHOUSE_BITBUCKET_API_TOKEN`.
...
- The [`ci_timeout` reference](/gatehouse/reference/global-config/#ci_timeout) owns idle re-arming, unlimited monitoring, and fail-closed reconciliation while that gate is parked
...
- Once every check has finished, classifies each terminally failed check by the provider's own reported outcome before anything escalates; [`ci.rerun_transient`](/gatehouse/reference/repo-config/#cirerun_transient) owns which outcomes count as the provider reporting itself
