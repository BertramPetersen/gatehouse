...
func TestReadSurfaceGate_ConcurrentProcessesEmitOnce(t *testing.T) {
	const helperEnv = "GATEHOUSE_READ_GATE_HELPER"
	if os.Getenv(helperEnv) == "1" {
		gate := NewReadSurfaceGate(os.Getenv("GATEHOUSE_READ_GATE_PATH"), 10*time.Minute, func() time.Time {
	...
	for range callers {
		go func() {
			...
			cmd.Env = append(os.Environ(), helperEnv+"=1", "GATEHOUSE_READ_GATE_PATH="+path)
