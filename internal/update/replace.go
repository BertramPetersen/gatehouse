...
func replaceExecutable(target string, binaryData []byte) error {
	...
	if err := replaceExecutableAtomically(target, binaryData, perm); err == nil {
		...
		return fmt.Errorf("self-update requires an atomic replace on macOS; reinstall gatehouse so the PATH entry points at a user-owned binary, then retry update: %w", err)
	}
