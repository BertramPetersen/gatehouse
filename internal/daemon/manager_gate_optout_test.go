...
// given .gatehouse.yaml (empty string => no file), plus a linked worktree with
...
func gateOptOutWorktree(t *testing.T, repoYAML string) (string, string) {
	...
	if repoYAML != "" {
		if err := os.WriteFile(filepath.Join(src, ".gatehouse.yaml"), []byte(repoYAML), 0o644); err != nil {
			t.Fatal(err)
		}
	}
	gitCmd(t, src, "add", ".")
...
// .gatehouse.yaml, which is NOT opted out and must NOT abort.
...
func TestAssertGateTrustedConfigReadable_PresentUnreadableBlobAborts(t *testing.T) {
	...
	cmd.Dir = wt
	cmd.Stdin = strings.NewReader("100644 blob " + blobSHA + "\t.gatehouse.yaml\n")
...
// malformed trusted .gatehouse.yaml aborts (we cannot evaluate the boundary).
