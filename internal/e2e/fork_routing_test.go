...
func TestForkRouting(t *testing.T) {
	...
	parentURL := "https://github.com/parent-owner/gatehouse.git"
	forkURL := "https://github.com/fork-owner/gatehouse.git"
	...
	t.Setenv("FAKEAGENT_GH_PARENT", "parent-owner/gatehouse")
	...
	if run.PRURL == nil || !strings.HasPrefix(*run.PRURL, "https://github.com/parent-owner/gatehouse/pull/") {
	...
	for _, inv := range invocations {
		...
		if len(inv.Args) >= 2 && inv.Args[0] == "pr" && inv.Args[1] == "create" {
			if inv.Repo == "fork-owner/gatehouse" {
				t.Fatalf("created silent self-PR against fork: %+v", inv)
			}
			if inv.Repo == "parent-owner/gatehouse" && inv.Head == "fork-owner:"+branch && inv.Base == "main" {
