...
func TestDaemonRunUsesProvidedRoot(t *testing.T) {
	...
	defer func() {
		_, _ = h.RunInDirWithEnv(h.WorkDir, map[string]string{"GATEHOUSE_HOME": wantRoot}, "daemon", "stop")
		cancel()
		select {
		case <-done:
	...
	for {
		...
		status, err := h.RunInDirWithEnv(h.WorkDir, map[string]string{"GATEHOUSE_HOME": wantRoot}, "daemon", "status")
	...
	if strings.Contains(defaultStatus, "daemon running") {
		t.Fatalf("daemon run --root should not use default GATEHOUSE_HOME, got status %q", defaultStatus)
	}
}
