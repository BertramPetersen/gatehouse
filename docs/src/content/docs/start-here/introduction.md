---
title: Introduction
description: What gatehouse is and why it exists.
---

`gatehouse` puts a local git proxy in front of your real remote. Push to
`gatehouse` instead of `origin`, and it spins up a disposable worktree, runs
...
`gatehouse` sits in that gap. It gives you a deliberate local gate before the
...
`gatehouse` is a separate remote on purpose.

- `origin` is never rewritten or hijacked.
- `git push origin` still behaves exactly like normal Git.
- `git push gatehouse` is an explicit signal that this branch should go
...
```mermaid
flowchart LR
  repo["Your repo"] -->|"git push gatehouse"| gate["Local gate repo"]
...
into the gate by pushing to the `gatehouse` remote.
...
- A `/gatehouse` agent skill so a coding agent can do a task and gate it, or gate existing committed work, backed by a non-interactive `gatehouse axi` interface.
- A setup wizard when you run bare `gatehouse` with no active run on the current branch - it walks you through creating a branch, committing, and pushing through the gate, then attaches if the daemon registers the new run.
...
- **`git push gatehouse`** - the explicit Git path. You push a committed branch to the gate remote instead of `origin`, and the daemon takes it from there. See [Quick Start](/gatehouse/start-here/quick-start/).
- **`gatehouse`** - the terminal UI. Run it after making changes and a [setup wizard](/gatehouse/guides/setup-wizard/) walks you through branch, commit, and push, then attaches to the live run so you can watch, approve, fix, skip, or abort each step.
- **`/gatehouse`** - the agent skill. Tell a coding agent `/gatehouse <task>` to have it do the task, commit it on a feature branch, and then gate it with that task as intent; use bare `/gatehouse` to gate existing committed work. It resolves safe findings on its own and stops to relay anything that needs your decision. See [Driving gatehouse as an agent](/gatehouse/guides/agents/#driving-gatehouse-as-an-agent).

`gatehouse init` installs the `/gatehouse` skill at user level for its supported driving-agent locations; Grok Build is a pipeline runner and does not consume that skill. The skill drives `gatehouse axi`, a non-interactive command surface that prints [TOON](https://toonformat.dev) to stdout, so an agent reaches the same gate and the same approval points you get in the TUI. See [Driving gatehouse as an agent](/gatehouse/guides/agents/#driving-gatehouse-as-an-agent) for the supported consumers.

## Next

- [Quick Start](/gatehouse/start-here/quick-start/) - first push in five minutes
- [Installation](/gatehouse/start-here/installation/) - full install options
- [The Gate Model](/gatehouse/concepts/gate-model/) - architecture and design decisions
