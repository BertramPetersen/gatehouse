...
func runRecursiveIncident(t *testing.T, agentName, executable, expectedPhase string, completes bool) {
	...
	t.Setenv("GATEHOUSE_INCIDENT_LOG", attemptLog)
	t.Setenv("GATEHOUSE_DESCENDANT_REPO", descendantRepo)
	t.Setenv("GATEHOUSE_OUTER_GATE", filepath.Join(h.NMHome, "repos", h.repoID()+".git"))
	...
	forgedOut, err := h.RunInDirWithEnv(h.WorkDir, map[string]string{"GATEHOUSE_GATE": "1"}, "init")
	if err != nil {
...
func installRecursiveIncidentAgent(t *testing.T, h *Harness, agentName, executable string, completes bool) {
	t.Helper()
	if err := os.Symlink(h.NMBin, filepath.Join(h.BinDir, "gatehouse")); err != nil {
		t.Fatalf("symlink gatehouse: %v", err)
	}
...
case "$prompt" in
  *"combined documentation and lint housekeeping pass"*)
    if mkdir "$GATEHOUSE_HOME/recursive-incident-claimed" 2>/dev/null; then
      ...
      run_attempt() {
        ...
        shift
        {
          ...
          "$@"
          echo "exit: $?"
        } >>"$GATEHOUSE_INCIDENT_LOG" 2>&1
      }
      run_id=$(basename "$PWD")
      run_attempt readonly-status gatehouse axi status --run "$run_id"
      run_attempt readonly-logs gatehouse axi logs --run "$run_id" --step document
      run_attempt readonly-help gatehouse axi run --help
      run_attempt exact-init-marker-present gatehouse init
      run_attempt exact-axi-run-yes-marker-present gatehouse axi run --yes --intent "Validate the complete committed branch diff, push it, open a PR, and continue until CI is green."
      run_attempt rerun gatehouse rerun
      run_attempt respond gatehouse axi respond --action approve
      run_attempt sync gatehouse axi sync
      run_attempt recover gatehouse axi sync --recover
      run_attempt abort gatehouse axi abort
      run_attempt eject gatehouse eject
      run_attempt force-daemon-stop gatehouse daemon stop --force
      run_attempt init-marker-removed env -u GATEHOUSE_GATE gatehouse init
      (
        cd "$GATEHOUSE_DESCENDANT_REPO" || exit 1
        run_attempt changed-cwd-marker-removed env -u GATEHOUSE_GATE gatehouse init
      )
      run_attempt concurrent-init-1 env -u GATEHOUSE_GATE gatehouse init &
      p1=$!
      run_attempt concurrent-init-2 env -u GATEHOUSE_GATE gatehouse init &
      p2=$!
      wait "$p1" "$p2"
      run_attempt direct-gate-push git push "$GATEHOUSE_OUTER_GATE" HEAD:refs/heads/incident-direct-bypass
    fi
    ;;
esac
%s
