...
func TestStatsCommandRendersAllRepoDashboard(t *testing.T) {
	...
	for _, notWant := range []string{"Saved", "Rescue runs", "Mistakes fixed", "auto-fix", "caught in review", "╭─ gatehouse"} {
...
func TestStatsDashboardTopBorderShowsGitPushGatehouse(t *testing.T) {
	out := renderStatsDashboard(&db.Stats{})
	firstLine := strings.Split(out, "\n")[0]
	if !strings.Contains(firstLine, "git push gatehouse") {
...
func TestStatsDashboardCentersBannerAsBlock(t *testing.T) {
	...
	for _, line := range lines {
		if slices.ContainsFunc(strings.Split(banner, "\n"), func(row string) bool { return strings.Contains(line, row) }) {
...
func TestStatsDashboardStylesBannerAndProgressBars(t *testing.T) {
	...
	if !strings.Contains(out, sCyan.Render(strings.Split(banner, "\n")[0])) {
