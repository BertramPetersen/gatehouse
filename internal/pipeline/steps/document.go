...
func (s *DocumentStep) Execute(sctx *pipeline.StepContext) (*pipeline.StepOutcome, error) {
	...
	combinedLint := sctx.Config.Commands.Lint == "" && sctx.Config.SameStepProfiles(types.StepDocument, types.StepLint)
	if combinedLint {
...
// .gatehouse.yaml (config.EffectiveRepoConfig), so a contributor's pushed
