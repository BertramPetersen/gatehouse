...
import (
	...
	"github.com/BertramPetersen/gatehouse/internal/tui"
...
// both the root command (bare `gatehouse`) and the `attach` subcommand.
func attachRun(ctx context.Context, w io.Writer, runID string, rootDefault bool, autoYes bool, skipSteps []types.StepName) error {
	...
	if run == nil {
		// No active run - if the user ran bare `gatehouse` in their repo
	...
	if repoID != "" {
		...
		if err == nil && len(runs) > 0 {
			...
			if len(runs) > recentRunsLimit {
				fmt.Fprintf(w, "  %s\n", sDim.Render(fmt.Sprintf("(%d more - run 'gatehouse runs' to see all)", len(runs)-recentRunsLimit)))
			}
			fmt.Fprintln(w)
