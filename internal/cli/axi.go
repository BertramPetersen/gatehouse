...
// /gatehouse skill) drives; humans use the bare `gatehouse` TUI instead.
func newAxiCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "axi",
		Short: "Agent interface: drive gatehouse from an autonomous agent",
		Long: "Agent eXperience Interface for gatehouse. Prints token-efficient TOON\n" +
			"to stdout and is driven entirely by flags (no interactive prompts).\n" +
			"Running `gatehouse axi` with no subcommand shows the current state.",
		Args:          cobra.NoArgs,
		SilenceErrors: true,
		SilenceUsage:  true,
	...
	cmd.AddCommand(newAxiModelsCmd())
	return cmd
}
...
func runAxiHome(cmd *cobra.Command) (string, error) {
	...
	case currentActive == nil:
		help = append(help, `Run `+"`"+`gatehouse axi run --intent "<what the user set out to accomplish>"`+"`"+` to validate your changes`)
		if otherActive != nil {
	...
	case gated:
		help = append(help, "Run `gatehouse axi respond --action approve` to clear the current gate")
	default:
		help = append(help, "Run `gatehouse axi status` to inspect the active run")
	}
	if hasBranchSync {
		help = append(help, branchSyncAgentGuidance)
	}
	help = append(help, preserveGateFixCommitsGuidance)
	help = append(help, "The calling agent drives AXI gates but does not replace the configured pipeline agent; run `gatehouse doctor` if no native agent or ACP runner is available")
	help = append(help, "How to drive the pipeline: `gatehouse axi run --help`, or the `/gatehouse` skill (loaded when you invoke `/gatehouse`)")
...
func repoInitHelp(err error) []string {
	if err != nil && strings.Contains(err.Error(), "not initialized") {
		return []string{"Run `gatehouse init` to set up the gate in this repository"}
	}
	return nil
}
